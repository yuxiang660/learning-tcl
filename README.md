...
## Tcl命令解析规则
* Tcl解析一条命令时，只从左向右解析一次，进行一轮替换。每一个字符只会被扫描一次。
* 每一个字符只会发生一层替换，而不会对替换后的结果再进行一次扫描替换。
* Tcl解析后的代码有时候会当作参数传递给命令，有些命令会自己再次扫描替换，其实也是交给Tcl解析器再处理一次。
* 例子：扫描替换
```sh
set name profits
set num 11.1
format {$name : %.2f} $num # $name : 11.10
format "$name : %.2f" $num # profits : 11.10
```